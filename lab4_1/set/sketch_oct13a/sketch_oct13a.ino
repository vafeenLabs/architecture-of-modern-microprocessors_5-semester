...
void loop() {
  ...
  frequency = map(frequency,350,1570,0,255);    // Преобразование значения частоты в модель RGB от 0 до 255
  ...
  frequency = pulseIn(sensorOut, LOW);        // Чтение выходной частоты
  frequency = map(frequency,430,2050,10,155);  // Преобразование значения частоты в модель RGB от 0 до 255
  ...
  digitalWrite(S3,HIGH);        // Вывод S3 в HIGH
  frequency = pulseIn(sensorOut, LOW);        // Чтение выходной частоты
  frequency = map(frequency,113,600,0,2000);  // Преобразование значения частоты в модель RGB от 0 до 255
